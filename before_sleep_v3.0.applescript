...
set tmp_var to do shell script "pmset -g | grep hibernatemode"
...
if chargeLeft - 0 < 8 then
	...
	if hibernatemode is not equal to "0" then
		do shell script "sudo " & project_path & "pmset -a hibernatemode 0"
		-- LOG
	...
	do shell script "sudo " & project_path & "pmset schedule wake \"" & nextWakeTime & "\""
	-- LOG
